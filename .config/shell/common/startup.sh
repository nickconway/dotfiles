...
if [[ -n "$PS1" ]] && [[ -z $TMUX ]] && [[ -z $ZELLIJ ]]; then
    if [[ -z $SSH_CONNECTION ]]; then
        command -v tmux > /dev/null && tmux new -e SSH_CONNECTION=$SSH_CONNECTION -As main $TMUX_SHELL && exit 0
        # zellij a -c main
    else
        # zellij a -c ssh; exit
        command -v tmux > /dev/null && tmux new -e SSH_CONNECTION=$SSH_CONNECTION -As ssh $TMUX_SHELL && exit 0
    fi
fi
